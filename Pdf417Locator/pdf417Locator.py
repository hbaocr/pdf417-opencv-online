...
def loadImgBuffer2Gray(fileBuff):
    img_array = np.frombuffer(fileBuff, dtype=np.uint8)
    img = cv2.imdecode(img_array,cv2.IMREAD_COLOR)
    gray_img= cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)
    return gray_img


def loadImg2Gray(filename):
    img = cv2.imread(filename)
    gray_img= cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)
    return gray_img

def saveBuff2Jpg(img_buff,fname):
   cv2.imwrite(fname,img_buff)
...
#f= open("t4.jpg", "rb")
#fb = f.read()
#gray=loadImgBuffer2Gray(fb)
#child=locatePDF417(gray)
#print(child[0].shape)
#saveBuff2Jpg(child[0],'1.jpg')